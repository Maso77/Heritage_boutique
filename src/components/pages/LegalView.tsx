...
import { LEGAL_FALLBACK_PAGES, LegalFallbackPage, LegalPageKey } from './legalFallbackContent';
...
type LegalPage = LegalFallbackPage;
...
export const LegalView: React.FC<LegalViewProps> = ({ type, navigate }) => {
  ...
  const displayedPage = page || LEGAL_FALLBACK_PAGES[type];
  ...
  return (
    <div className="bg-[#FAF9F7] min-h-screen pt-24 pb-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!loaded ? <div className="bg-white border border-[#002141]/10 p-8 sm:p-12 text-sm text-[#3A3A3A]">Chargement…</div> : displayedPage ? (
          <article className="bg-white border border-[#002141]/10 p-8 sm:p-12 space-y-8 text-xs sm:text-sm text-[#3A3A3A] leading-relaxed">
            <h1 className="font-playfair text-2xl sm:text-3xl font-bold text-[#002141]">{displayedPage.title}</h1>
            <div className="[&_h2]:font-playfair [&_h2]:text-base [&_h2]:font-bold [&_h2]:text-[#002141] [&_h2]:mt-7 [&_p]:mb-4 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5" dangerouslySetInnerHTML={{ __html: displayedPage.content_html }} />
            <p className="border-t border-[#002141]/10 pt-5 text-[11px] text-[#3A3A3A]/70">Dernière mise à jour : {new Date(displayedPage.updated_at).toLocaleDateString('fr-FR')}</p>
          </article>
        ) : (
